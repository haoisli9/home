...
"#B5A1FF
hi cFunction        gui=NONE guifg=#C488FF ctermfg=green
...
" Math Operator
hi cPointerOperator         guifg=#3EFFE2 ctermfg=yellow
hi cLogicalOperator         guifg=#3EFFE2 ctermfg=yellow
hi cBinaryOperator          guifg=#3EFFE2 ctermfg=yellow
hi cBinaryOperatorError     guifg=#3EFFE2 ctermfg=yellow 
hi cLogicalOperator         guifg=#3EFFE2 ctermfg=yellow
hi cLogicalOperatorError    guifg=#3EFFE2 ctermfg=yellow

syn match  cHiOperator1  display "[^=]=[^=]"ms=s+1,me=e-1
syn match  cHiOperator2  display "[\+\-]\{1,2}[^>]"me=e-1
syn match  cHiOperator3  display "[^\/\*][\*\/][^\*\/]"ms=s+1,me=e-1
hi cHiOperator1   guifg=lightmagenta ctermfg=yellow
hi cHiOperator2   guifg=lightmagenta ctermfg=yellow
hi cHiOperator3   guifg=lightmagenta ctermfg=yellow

"parenthesis brackets brace
syn match  cParenthesis display "[()]"
syn match  cBrace display "[{}]"
syn match  cBrackets display "[\[\]]"

"hi cParenthesis  guifg=green1
"hi cBrace guifg=orange1
"hi cBrackets guifg=yellow1

" lihao --- add structure highlight"
syn match  cMyStruct display  "^\s*\<[a-zA-Z_][a-zA-Z_0-9]*\>[\*]\{,2}\s*[a-zA-Z_]"me=e-1 contains=cType,cStatement,cMyType
syn match  cMyStruct display  "^\s*\<[a-zA-Z_][a-zA-Z_0-9]*\>\s*[\*]\{1,2}\s*[a-zA-Z_]"me=e-1 contains=cType,cStatement,cMyType
hi def link cMyStruct cStructure

syn match  cCaseConst	display "^\s*case\s*\<[a-zA-Z_0-9]*\>:" contains=cLabel
hi def link cCaseConst  cDefine

" lihao --- MY KEYWORD DEFINE
syn keyword cMyType     UINT8 UINT16 UINT32 INT8 INT16 INT32 VOID
hi  cMyType guifg=yellowgreen ctermfg=yellow

syn region  cIfZero  start="^\s*#\s*if\s\+[(]\=0[)]\=\>" end="^\s*#\s*endif" contains=cComment
hi def link  cIfZero cComment
