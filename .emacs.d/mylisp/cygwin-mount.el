...
;; add by lihao to handle Msys2 directory format like "/d/Users/"
(defconst cygwin-mount-cygwin-msys2-regexp "^/[A-Za-z]/")
(defun cygwin-convert-msys2-file-cmd (path)
  "Convert path from CYGWIN UNIX style to Windows style"
  (if (string-match cygwin-mount-cygwin-msys2-regexp path)
      (replace-regexp-in-string "\n" "" (shell-command-to-string (concat "cygpath -w " path)))
    (path)))

(defun cygwin-convert-msys2-file (name)
  "Convert Msys2 path style to Windows style."
  ;; (message "%S" name)
  ;; (message "%S" (concat (substring name 1 2) ":" (substring name 2)))
  (save-match-data
    (cond ((string-match cygwin-mount-cygwin-msys2-regexp name)
           (concat (substring name 1 2) ":" (substring name 2)))
          (t name))))
