...
(defcustom sdcv-dictionary-list nil
...
(defun sdcv-search-current-word ()
...
(defun sdcv-generate-dictionary-argument ()
  "Generate dictionary argument for sdcv from `sdcv-dictionary-list'"
  (if (null sdcv-dictionary-list)
      (format " --data-dir %s " sdcv-dictionary-data-dir)
    (format " %s --data-dir %s "
            ;; (mapconcat (lambda (dict)
            ;;              (concat "-u \"" dict "\""))
            ;;            sdcv-dictionary-list " ")
...
(defvar sdcv-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "o" 'show-entry)
    (define-key map "z" 'hide-entry)
    map)
