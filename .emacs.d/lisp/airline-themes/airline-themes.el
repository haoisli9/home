          ...
          (concat
           ...
           (if (eq airline-display-directory 'airline-directory-shortened)
               (propertize
                ...
                'face `(:foreground ,(face-foreground 'airline-normal-inner)
                        :background ,(face-background 'airline-normal-inner)))
             (propertize
           ...
           (propertize
            (concat (char-to-string airline-utf-glyph-separator-left) " ")
            'face `(:foreground ,(face-background 'airline-normal-inner)
                    :background ,(face-background 'airline-normal-center)))

           (propertize
            (concat (airline-curr-dir-git-branch-string (eshell/pwd)) " ")
            'face `(:foreground ,(face-foreground 'airline-normal-center)
                    :background ,(face-background 'airline-normal-center)))

           (propertize
            (concat (char-to-string airline-utf-glyph-separator-left))
            'face `(:foreground ,(face-background 'airline-normal-center)))
          ...
          (lhs-mode (when (or (not airline-hide-state-on-inactive-buffers)
                      ...
                      (if evil-mode-active
                          (list
                           ...
                           ;; Modified string
                           (powerline-raw mode-line-modified inner-face 'l)
                           (powerline-raw mode-line-client inner-face)
                           (powerline-raw mode-line-remote inner-face))
                        (list
                         ;; Modified string
                         (powerline-raw mode-line-modified outer-face 'l)
                         (powerline-raw mode-line-client out-face)
                         (powerline-raw mode-line-remote out-face)
                         ;; Separator >
